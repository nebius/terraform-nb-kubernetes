...
resource "nebius_kms_symmetric_key" "kms_key" {
...
resource "nebius_kms_symmetric_key_iam_binding" "encrypter_decrypter" {
  count            = var.create_kms ? 1 : 0
  symmetric_key_id = nebius_kms_symmetric_key.kms_key[count.index].id
  role             = "kms.keys.encrypterDecrypter"
  members = [
    "serviceAccount:${nebius_iam_service_account.master.id}",
  ]
}
